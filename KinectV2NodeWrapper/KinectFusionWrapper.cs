/// This code is based on the KinectFusionExplorer sample from the Kinect SDK
/// 
using System;
...
using KinectV2NodeWrapper.Support;
using System.IO;
...
{
    public class KinectFusionWrapper
    {
        #region variables
        /// <summary>
        ...
        /// </summary>
        private Matrix4 virtualCameraWorldToCameraMatrix4 = Matrix4.Identity;
        ...
        #region events
        public delegate void FrameCapturedListener(ref int[] pixels);

        public event FrameCapturedListener OnReconstructionFrame;
        #endregion
        private static KinectFusionWrapper singleton;

        public static KinectFusionWrapper GetInstance()
        {
            if (singleton == null)
            {
                singleton = new KinectFusionWrapper();
        ...
        /// <summary>
        /// Contains the representation of the joints in the 3d space
        /// </summary>
        private Dictionary<ulong, Body3D> bodies3D;
        ...
        public bool KinectView
        {
            ...
            set
            {
                ...
                if (this.kinectView)
                {
                    ...
                    this.virtualCamera.Detach(this);
                }
                else
                {
                    this.virtualCamera.Attach(this);
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!this.disposed)
            {
                if (disposing)
                {
                    ...
                    {
                        this.virtualCamera.CameraTransformationChanged -= this.OnVirtualCameraTransformationChanged;
                        this.virtualCamera.Detach(this);     // Stop getting mouse events from the image
        ...
        private static void RenderColorImage(FusionColorImageFrame colorFrame, ref int[] colorPixels)
        {
        ...
        System.Windows.Threading.Dispatcher mainDispatcher = System.Windows.Threading.Dispatcher.CurrentDispatcher;
        public void Start()
        {
            ...
            catch (IndexOutOfRangeException)
            {
                ...
                Console.WriteLine("No DirectX11 capable device installed");
                return;
            }
            catch (DllNotFoundException)
            {
                Console.WriteLine("Dlls not Found");
                return;
            }
            ...
            this.bodies3D = new Dictionary<ulong, Body3D>();
            ...
            this.mapper = sensor.GetCoordinateMapper();

            MultiSourceFrameReader reader = sensor.GetMultiSourceFrameReader();

            FrameDescription depthFrameDescription = sensor.GetDepthFrameSource().FrameDescription;
            ...
            FrameDescription colorFrameDescription = sensor.GetColorFrameSource().FrameDescription;
            ...
            this.virtualCamera = new GraphicsCamera(this.virtualCameraStartTranslation, this.virtualCameraStartRotation, 844 / 780);
            ...
            reader.MultiSourceFrameArrived += this.Reader_MultiSourceFrameArrived;
        ...
        private List<ulong> removeBody3D = new List<ulong>();
        private bool pauseRecording = false;
        private void SkeletonView_OnBodyArrayChanged(object source, Body[] bodies)
        {
            ...
            foreach (Body body in bodies)
            {
                ...
                {
                    Body3D body3D;
                    if (!bodies3D.ContainsKey(body.TrackingId))
                    {

                        body3D = new Body3D();
                        bodies3D.Add(body.TrackingId, body3D);
                    }
                    else
                    {
                        body3D = bodies3D[body.TrackingId];
                    }
                    ...
                    foreach (JointType jointType in joints.Keys)
                    {
                        ...
                        TranslateTransform3D translate = new TranslateTransform3D(position.X, position.Y, position.Z);
                        transform.Children.Add(translate);
                        transform.Children.Add(new MatrixTransform3D(fixedRotY180));
                        transform.Children.Add(this.virtualCamera.cameraFrustumTransform3D);

                        if (joints[jointType].TrackingState == TrackingState.Tracked)
                        {
                            body3D.JointCloud[jointType].Add(transform.Transform(new Point3D(0, 0, 0)));
                        }
            ...
            removeBody3D.Clear();
            foreach (ulong key in bodies3D.Keys)
            {
                if (!ContainsBodyId(bodies, key))
                {
                    Body3D body3D = bodies3D[key];
                    removeBody3D.Add(key);
                }
            }

            foreach (ulong key in removeBody3D)
            {
                bodies3D.Remove(key);
            }
        ...
        private bool ContainsBodyId(Body[] bodies, ulong key)
        {
        ...
        private void TrackCamera()
        {
            ...
            if (!trackingSucceeded && 0 != this.successfulFrameCount)
            {
                ...
                if (!this.cameraPoseFinderAvailable)
                {
                    ...
                    Console.WriteLine(String.Format("{0} -> Impossible to find the camera pose", DateTime.Now));
                }
                else
                {
                    ...
                    {
                        // Show tracking error on status bar
                        Console.WriteLine(String.Format("{0} -> Impossible to track the camera", DateTime.Now));
                    }
                }
            }
            else
            {
                ...
                {
                    Console.WriteLine(String.Format("{0} -> Kinect Fusion camera tracking RECOVERED! Residual energy={1}", DateTime.Now, this.alignmentEnergy));
                }
            ...
            {
                // Bad tracking
                Console.WriteLine(String.Format("{0} -> Automatically reset volume", DateTime.Now));
            ...
            if (trackingSucceeded)
            {
                ...
                {
                    mainDispatcher.BeginInvoke(
                     (Action)(() => this.UpdateVirtualCameraTransform()));
        ...
        private bool FindCameraPoseAlignPointClouds()
        {
            ...
            {
                Console.WriteLine(String.Format("{0} -> Impossible to find enough matches", DateTime.Now));
                return false;
            }
        ...
        public void RenderReconstruction()
        {
            ...
            /*System.Windows.Threading.Dispatcher.CurrentDispatcher.BeginInvoke(
                     (Action)(() => this.ReconstructFrameComplete()));*/
        ...
        private void ReconstructFrameComplete()
        {
            ...
            if (this.OnReconstructionFrame != null)
            {
                this.OnReconstructionFrame(ref this.shadedSurfaceFramePixelsArgb);
            }
        }
        ...
        private void RenderAlignDeltasFloatImage(FusionFloatImageFrame alignDeltasFloatFrame)
        {
        ...
        /// </summary>
        private void RestartReconstruction()
        {
            if (null == KinectManager.GetInstance())
            {
                return;
            }
            ...
            if (null != this.volume)
            {
                ...
                {
                    Console.WriteLine(String.Format("{0} -> Impossible to reset the reconstruction", DateTime.Now));
                }
            }
        }
        ...
        public void ResetReconstruction()
        {
            ...
            foreach (Body3D body in bodies3D.Values)
            {
                body.ClearClouds();
            }
        }
        ...
        /// </summary>
        public void CreateMesh(string fileName, MeshFormat format)
        {
            ...
            try
            {
                ...
                switch (format)
                {
                    ...
                    case MeshFormat.Obj:
                        ...
                        foreach (Body3D body3D in bodies3D.Values)
                        {
                            using (StreamWriter writer = new StreamWriter(fileName.Replace(".obj", "") + String.Format("_skeleton{0}.obj", skId)))
                            {
                                WriteSkeleton(flipAxes, body3D, writer);
                            }
                            skId++;
                        }
                        break;
                    ...
                    case MeshFormat.Ply:
                        ...
                        foreach (Body3D body3D in bodies3D.Values)
                        {
                            using (StreamWriter writer = new StreamWriter(fileName.Replace(".ply", "") + String.Format("_skeleton{0}.obj", skId)))
                            {
                                WriteSkeleton(flipAxes, body3D, writer);
                            }
                            skId++;
            ...
            {
                Console.WriteLine(string.Format("{0} -> Error saving Mesh", DateTime.Now));
            }
            catch (InvalidOperationException)
            {
                Console.WriteLine(string.Format("{0} -> Error saving Mesh", DateTime.Now));
            ...
            {
                Console.WriteLine(string.Format("{0} -> Out of memory", DateTime.Now));
            }
            finally
            {
        ...
        private static void WriteSkeleton(bool flipAxes, Body3D body3D, StreamWriter writer)
        {
            // Write the header lines
            writer.WriteLine("#");
            writer.WriteLine("# OBJ file created by Microsoft Kinect Fusion");
            writer.WriteLine("#");
            writer.WriteLine("# Kinect joint -> Web standard color table");
            writer.WriteLine("#");
            writer.WriteLine("# Spine and Head");
            writer.WriteLine("# Head -> MidnightBlue");
            writer.WriteLine("# Neck -> CornflowerBlue ");
            writer.WriteLine("# SpineShoulde -> Navy");
            writer.WriteLine("# SpineMid -> LightSteelBlue");
            writer.WriteLine("# SpineBase -> MediumBlue");
            writer.WriteLine("#");
            writer.WriteLine("# Right Arm");
            writer.WriteLine("# ShoulderRight, Colors.GreenYellow");
            writer.WriteLine("# ElbowRight -> LimeGreen");
            writer.WriteLine("# WristRight -> MediumSpringGreen");
            writer.WriteLine("# HandRight -> ForestGreen");
            writer.WriteLine("# ThumbRight -> OliveDrab");
            writer.WriteLine("# HandTipRight -> SeaGreen");
            writer.WriteLine("#");
            writer.WriteLine("# Left Arm");
            writer.WriteLine("# ShoulderLeft -> Thistle");
            writer.WriteLine("# ElbowLeft -> Fuchsia");
            writer.WriteLine("# WristLeft -> MediumOrchid");
            writer.WriteLine("# HandLeft -> DarkMagenta");
            writer.WriteLine("# ThumbLeft -> MediumSlateBlue");
            writer.WriteLine("# HandTipLeft -> Indigo");
            writer.WriteLine("#");
            writer.WriteLine("# Right Leg");
            writer.WriteLine("# HipRight -> Salmon");
            writer.WriteLine("# KneeRight -> LightCoral");
            writer.WriteLine("# AnkleRight -> Crimson");
            writer.WriteLine("# FootRight -> Firebrick");
            writer.WriteLine("#");
            writer.WriteLine("# Left Leg");
            writer.WriteLine("# HipLeft -> Gold");
            writer.WriteLine("# KneeLeft -> Yellow");
            writer.WriteLine("# AnkleLeft -> Moccasin");
            writer.WriteLine("# FootLeft -> DarkKhaki");


            foreach (JointType jointType in Enum.GetValues(typeof(JointType)))
            {
                Color c = Body3D.CloudColor[jointType];
                foreach (Point3D p in body3D.JointCloud[jointType])
                {


                    writer.WriteLine(String.Format(CultureInfo.InvariantCulture, "v {0:0.0000000} {1:0.0000000} {2:0.0000000} {3} {4} {5}",
                        p.X,
                        flipAxes ? -p.Y : p.Y,
                        flipAxes ? -p.Z : p.Z,
                        c.ScR,
        ...
        public void VolumeSettingsChanged()
        {
        ...
        private void UpdateAlignDeltas()
        {
            ...
            {
                mainDispatcher.BeginInvoke(
                     (Action)(() => this.RenderAlignDeltasFloatImage(
                            this.deltaFromReferenceFrame)));
            }
        ...
        public void SaveBitmap(string filename)
        {
            ...
            WriteableBitmap bitmap = new WriteableBitmap(shadedSurfaceNormalsFrame.Width,
                shadedSurfaceNormalsFrame.Height, 96.0, 96.0, PixelFormats.Bgr32, null);
